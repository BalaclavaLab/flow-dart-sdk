...
Uint8List signData(List<int> input, String privateKey, String domainTag) {
  ...
  var privateParameters = new PrivateKeyParameter(
      new ECPrivateKey(BigInt.parse(privateKey, radix: 16), new ECDomainParameters('prime256v1')));

  var signatureParams = () => new ParametersWithRandom(privateParameters, new NullSecureRandom());
  ...
  return signatureToUint8List(signer.generateSignature(digest) as ECSignature);
}

// Return signature from here
void signPayload(Transaction transaction, List<int> payload, List<int> signerAddress,
    String privateKey, int keyId, List<dynamic> payloadSignatureHolder) {
  var previousSignatures =
      transaction.payloadSignatures.where((element) => element.address == signerAddress);
  ...
  final selected =
      transaction.payloadSignatures.indexWhere((signature) => signature.address == signerAddress);

  payloadSignatureHolder.add([selected, keyId, String.fromCharCodes(signature)]);
}
